  ...
  <div class="real-container">
    ...
    <b-row class="ui-row grid-section">
      <b-col sm="6" class="display-flex flex-wrap ui-col">
        <h3 class="grid-tit">배송 예정 데이터 조회</h3>
      </b-col>
    ...
    >
      <b-row class="modal-row">
        <b-col sm="6" class="modal-col">
          <span class="modal-tit">
            화주
          </span>
          <multiselect
          v-model="infoModal.selectFirstVal"
          :options="locationOption"
          :multiple="false"
          selectLabel="추가"
          deselectLabel="제외"
          placeholder="배송지를 선택해주세요."
          class="default-v-select"
          track-by="name"
          label="name"><span slot="noResult">일치하는 배송지가 없습니다.</span></multiselect>
        </b-col>
        <b-col sm="6" class="modal-col">
          ...
          <multiselect
          v-model="infoModal.selectSecondVal"
          :options="locationOption"
          :multiple="true"
          selectLabel="추가"
          deselectLabel="제외"
      ...
      <b-row class="modal-row">
        <b-col sm="6" class="modal-col">
          ...
          <multiselect
          v-model="infoModal.selectThirdVal"
          :options="locationOption"
          :multiple="false"
          selectLabel="추가"
          deselectLabel="제외"
        ...
        <b-col sm="6" class="modal-col">
          ...
          <multiselect
          v-model="infoModal.selectFourthVal"
          :options="locationOption"
          :multiple="false"
          selectLabel="추가"
          deselectLabel="제외"
      ...
      <b-row class="modal-row modal-grid-row">
        <b-col sm="6" class="modal-col half-col left">
          <span class="grid-tit">알콘</span>
          <div id="modal-left-grid" class="real-grid col-grid"></div>
        </b-col>
        <div class="modal-grid-control">
          <button type="button" @click="gridMoveLeft" class="move-btn">
            <b-icon-arrow-left-circle-fill></b-icon-arrow-left-circle-fill>
          </button>
          <button type="button" @click="gridMoveRight" class="move-btn">
            <b-icon-arrow-right-circle-fill></b-icon-arrow-right-circle-fill>
          </button>
          <button type="button" @click="gridMoveAll" class="move-btn">
            <b-icon-chevron-double-right></b-icon-chevron-double-right>
          </button>
        </div>
        <b-col sm="6" class="modal-col half-col right">
          <span class="grid-tit">알콘</span>
          <div id="modal-right-grid" class="real-grid col-grid"></div>
        </b-col>
      </b-row>
...
export default {
  ...
  methods: {
    ...
    addRow() {
      ...
      this.$root.$emit('bv::show::modal', 'scheduled-modal')
      setTimeout(() => {
        this.modalLeftData()
        this.modalRightData()
      }, 100)
    ...
    addSave() {
      this.$nextTick(() => {
        this.$bvModal.hide('scheduled-modal')
        let values = [
          this.infoModal.name,
    ...
    modalLeftData() {
      ...
      gridView = new GridView('modal-left-grid')
    ...
    modalRightData() {
      dataProvider = new LocalDataProvider(false)
      gridView = new GridView('modal-right-grid')
      gridView.setDataSource(dataProvider)
      gridView.setCheckBar({
        visible: false
      })
